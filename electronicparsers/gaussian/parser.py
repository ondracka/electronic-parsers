...
class GaussianOutParser(TextParser):
    ...
    def init_quantities(self):
        ...
        run_quantities = [
            ...
            ),  # when x_gaussian_settings_corrected is no longer stored, `strip()` can be removed
...
class GaussianParser:
    ...
    def parse_scc(self, section):
        ...
        # orbital symmetries
        orbital_symmetries = section.get('population_analysis', {}).get(
            'orbital_symmetries'
        )
        if orbital_symmetries is not None:
            ...
            if orbital_symmetries[:2]:
                sec_orbital_symmetries.x_gaussian_alpha_symmetries = np.concatenate(
                    orbital_symmetries[:2]
                )
            if orbital_symmetries[2:]:
                sec_orbital_symmetries.x_gaussian_beta_symmetries = np.concatenate(
                    orbital_symmetries[2:]
                )

        # electronic symmetries
        elstate_symmetry = section.get('population_analysis', {}).get(
            'x_gaussian_elstate_symmetry'
        )
        ...
        if optimization_completed is not None:
            ...
            sec_run.x_gaussian_section_geometry_optimization_info.append(
                sec_optimization_info
            )
            sec_optimization_info.x_gaussian_geometry_optimization_converged = (
                optimization_completed
            )
        ...
        if multipoles:
            ...
            sec_scc.x_gaussian_section_molecular_multipoles.append(
                sec_molecular_multipoles
            )
            sec_molecular_multipoles.x_gaussian_molecular_multipole_values = multipoles
            sec_molecular_multipoles.x_gaussian_molecular_multipole_m_kind = (
                'polynomial'
            )
        ...
        if frequencies is not None:
            ...
            if normal_modes is not None:
                ...
                normal_modes = np.reshape(
                    normal_modes, (len(normal_modes), len(normal_modes[0]) // 3, 3)
                )
                sec_frequencies.x_gaussian_normal_mode_values = np.transpose(
                    normal_modes, axes=(1, 0, 2)
                )
        ...
        if temperature_pressure is not None:
            ...
            if section.get('zero_point_energy') is not None:
                sec_scc.energy.zero_point = EnergyEntry(
                    value=section.get('zero_point_energy')
                )
            ...
            for key in keys:
                ...
                if val is not None:
                    setattr(
                        sec_thermochem, 'x_gaussian_%s' % key, val.to('joule').magnitude
                    )
        ...
        if iteration is not None:
            ...
            if (
                iteration.get('x_gaussian_single_configuration_calculation_converged')
                is not None
            ):
                ...
                if energy_scf is not None and electronic_ke is not None:
                    sec_scf_energy.electrostatic = EnergyEntry(
                        value=energy_scf - electronic_ke
                    )
    ...
    def parse_method(self, n_run):
        ...
        def resolve_xc_functional(parameter):
            ...
            if res is not None:
                ...
                if len(xc_keys) != 2:
                    self.logger.warning(
                        'Cannot resolve xc functional', data=dict(key=name)
                    )
                    return
                ...
                x_name = (
                    'LC-'
                    if xc_keys[0] == 'LC-'
                    else self._xc_functional_map.get(xc_keys[0], [{}])[0].get(
                        'name', ''
                    )
                )
                c_index = -1 if xc_keys[0] == 'LC-' else 0
                c_name = self._xc_functional_map.get(xc_keys[1], [{}])[c_index].get(
                    'name', ''
                )
        ...
        if len(xc_functionals) != 1:
            self.logger.error(
                'Found multiple or no xc functional',
                data=dict(n_parsed=len(xc_functionals)),
            )
