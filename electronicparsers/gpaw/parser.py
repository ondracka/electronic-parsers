...
class GPWParser(TarParser):
    def __init__(self):
        ...
        self._info_map = {
            ...
            'atom_forces_free': 'CartesianForces',
            'atom_forces_free_raw': 'CartesianForces',
...
class GPAWParser:
    ...
    def parse_method(self):
        ...
        if smearing_width is not None:
            sec_electronic.smearing = Smearing(
                kind='fermi',
                width=self.apply_unit(smearing_width, 'energyunit')
                .to('joule')
                .magnitude,
            )
    ...
    def parse_scc(self):
        ...
        for key in energy_keys:
            ...
            if val is not None:
                sec_energy.m_add_sub_section(
                    ...
                    EnergyEntry(value=self.apply_unit(val, 'energyunit')),
                )
        ...
        if self.parser.get_array('atom_forces_free') is not None:
            ...
            raw = (
                self.parser.get_array('atom_forces_free_raw') * energyunit / lengthunit
            )
        ...
        if magnetic_moments is not None:
            ...
            sec_scc.x_gpaw_fixed_spin_Sz = magnetic_moments.sum() / 2.0
        ...
        if band_paths is not None:
            ...
            for band_path in band_paths:
                ...
                if band_path.get('eigenvalues', None) is not None:
                    energies = self.apply_unit(
                        band_path.get('eigenvalues'), 'energyunit'
                    )
        ...
        if density is not None:
            ...
            npoints = [
                (npt + 1) if not pbc[i] else npt for i, npt in enumerate(npoints)
            ]
            ...
            if potential is not None:
                ...
                sec_scc.potential[0].effective[0].displacements = (
                    displacements * lengthunit
                )
                sec_scc.potential[0].effective[0].value = (
                    potential * energyunit / lengthunit**3
                )
    ...
    def parse_system(self):
        ...
        pbc = (
            [True, True, True]
            if self.get_mode() == 'pw'
            else np.array(self.parser.get_array('boundaryconditions'), bool)
        )
        ...
        if momenta is not None:
            masses = np.array(
                [ase.data.atomic_masses[self.parser.get_array('atomicnumbers')]]
            )
            velocities = momenta / masses.reshape(-1, 1)
            sec_atoms.velocities = (
                velocities * ase.units.fs / ase.units.Angstrom * ureg.angstrom / ureg.fs
            )
