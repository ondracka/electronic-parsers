...
def test_graphite(parser):
    ...
    stress_tensor_first = run.calculation[0].stress.total.value
    ...
    assert np.allclose(
        stress_tensor_first, stress_tensor_expected_first, rtol=1e-8, atol=0.0
    )

    stress_tensor_last = run.calculation[-1].stress.total.value
    ...
    assert np.allclose(
        stress_tensor_last, stress_tensor_expected_last, rtol=1e-8, atol=0.0
    )
