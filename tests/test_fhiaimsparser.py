...
def test_hybrid_ignores_pre_scf_pbe(parser, tmp_path):
    """Keep HSE for the output pattern in Oasis entry 4gOJ_RakPsC-hgHMigxLDJrSAm_a."""
    source = Path('tests/data/fhiaims/GaAs_HSE06+SOC/aims.out').read_text()
    marker = '  XC: Using HSE-functional with OMEGA =  0.200000E+00 <units>.'
    assert source.count(marker) == 1
    mainfile = tmp_path / 'aims.out'
    mainfile.write_text(
        source.replace(
            marker,
            marker + '\n  Pre-SCF XC: Using PBE gradient-corrected functionals.',
        )
    )
    ...
    xc_functional = archive.run[0].method[0].dft.xc_functional
    assert [functional.name for functional in xc_functional.hybrid] == [
        'HYB_GGA_XC_HSE06'
    ]
    assert not xc_functional.exchange
    assert not xc_functional.correlation
